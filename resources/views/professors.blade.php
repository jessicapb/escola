...
<body class="bg-gray-50 min-h-screen p-8">
    <header class="mb-8 flex justify-between items-center">
        <h1 class="text-3xl font-semibold text-orange-400">Gestió de Professors</h1>
        <a href="{{ url('/') }}" class="text-orange-400 hover:underline">Tornar a l'inici</a>
    </header>
    ...
    <section class="mb-6 max-w-md">
        ...
        <input type="text" id="descripProfessor" placeholder="Descripció del professor" class="w-full border border-gray-300 rounded px-3 py-2 mb-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        
        <button 
            id="btnAfegir" 
            class="bg-orange-400 text-white px-4 py-2 rounded hover:bg-orange-600 transition w-full"
        >Afegir</button>
    </section>
    ...
    <script>
        ...
        async function carregarModuls(filtre = '') {
            ...
            moduls.forEach(modul => {
                const li = document.createElement('li');
                li.className = 'bg-white shadow rounded p-4 space-y-2'; // 'space-y-2' per espai entre elements
